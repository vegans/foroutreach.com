...
module.exports = {
  ...
  add: async (values) => {
    ...
    return Country.updateRelations({ _id: entry.id, values: relations });
  },
  ...
  edit: async (params, values) => {
    ...
    const entry = await Country.updateOne(params, data, { multi: true });

    // Update relational data and return the entry.
    return Country.updateRelations(Object.assign(params, { values: relations }));
  },
  ...
  remove: async params => {
    ...
    await Promise.all(
      Country.associations.map(async association => {
  ...
  search: async (params) => {
    ...
    const $or = Object.keys(Country.attributes).reduce((acc, curr) => {
      switch (Country.attributes[curr].type) {
        case 'integer':
        case 'float':
        case 'decimal':
